...
/**
 * An event exporter used to provide column labels for a <code>NewEvent</code>
...
public class NewEventLabelProvider extends AbstractEventLabelProvider implements NewEvent.Exporter {
	...
	public void addContourCreationRecords(List<ContourCreationRecord> creationRecords) {
		List<String> idList = new ArrayList<String>(creationRecords.size());
		for (ContourCreationRecord record : creationRecords) {
			record.export(fRecordLabelProvider);
			idList.add(fRecordLabelProvider.getContourID());
		}
		
		fObject = idList.toString();
	}
