...
import edu.buffalo.cse.jive.ui.search.queries.VariableChangedSearchQuery;

/**
 * An {@code IJiveSearchPage} used to create an
 * {@code VariableChangedSearchQuery}.  The page contains input fields for a
 * class, an optional instance number, an optional method name, a variable name,
 * and a (possibly empty) condition on the variable.
 * <p>
 * When an instance number is given, the condition is checked only for that
 * particular instance of the class.  When it is left out, the condition is
 * checked for every instance of the class.  If a method name is supplied, then
 * the variable is assumed to be a parameter or local variable of the method.
 * Otherwise, the variable is considered a field.
 * 
 * @see VariableChangedSearchQuery
 * @author Jeffrey K Czyz
 */
public class VariableChangedSearchPage extends AbstractJiveSearchPage {
	...
	public IJiveSearchQuery createSearchQuery() {
		...
		return new VariableChangedSearchQuery(pattern, operator, valueText.getText());
	}
